...
if __name__=='__main__':
    ...
    pickup_quality_knapsack = np.array([[[7 for i in range(clip_number)] for j in range(delta_d+1)] for k in range(O_v+1)])
            ...
            for c in range(1, clip_number+1):
                ...
                if candidatad_quality_arg.shape[0]>0:
                    ...
                    for q_i in range(len(candidatad_quality_arg)):
                        tmp_profit_matrix.append(opt_state[remain_time_idx[q_i]][remain_space_idx[q_i]][c-1] + profit_matrix[c-1][candidatad_quality_arg[q_i]])
                ...
                if tmp_max != 0:
                    ...
                    print("Before update:",c, pickup_quality_knapsack[o][delta])

                if opt_state[o][delta][c-1] >= tmp_max: # not pickup any quality for c
                    opt_state[o][delta][c] = opt_state[o][delta][c-1]
                    pickup_quality_knapsack[o][delta][c-1] = len(pre_d_selected)-1
                    print("not pickup any quality for c", opt_state[o][delta][c])
                else: # pick the length of c
                    opt_state[o][delta][c] = tmp_max
                    og_idx = candidatad_quality_arg[tmp_max_idx]
                    pickup_quality_knapsack[o][delta] = pickup_quality_knapsack[space_matrix[c-1][og_idx]][time_matrix[c-1][og_idx]]
                    pickup_quality_knapsack[o][delta][c-1] = candidatad_quality_arg[tmp_max_idx]

                    print("pickup quality c", opt_state[o][delta][c])
                if tmp_max != 0:
                    print("After update:",c,pickup_quality_knapsack[o][delta])

            
                

    pickup_quality = pickup_quality_knapsack[-1][-1].astype(int)
    time_sum = 0
